...
const Message = ({ chatMessage, messageType }) => { // Message = 채팅 한줄 한줄 
...
export default function Chat() {
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Escape') { // esc 누르면  꺼지게 하기
  ...
  useEffect(() => { // 메세지가 바뀌면, 채팅창을 켜면 ? 맨밑으로 이동
    scrollToBottom()
