...
import { getSellProduct, getSellProductCategori } from '../../../store/apis/product';
...
const StoreModal = () => {
  ...
  const { isLoading:ILC, data:characters } = useQuery<any>(
    ...
    async () => {return (await (getSellProductCategori(10)))
      },
    {
      onError: (err: any) => {
        console.log(err, "판매중 정보 실패");
  ...
  return (
    <Wrapper>
      <NavBar>
        <Button onClick={()=>{setMode('index')}}> 판매중인 NFT  </Button>
        <Button onClick={()=>{setMode('character')}}> 캐릭터 상점 </Button>
      </NavBar>
    {mode==='index'&&
    <div className='Index'>
