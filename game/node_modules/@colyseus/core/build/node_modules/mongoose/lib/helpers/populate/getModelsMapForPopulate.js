...
var SkipPopulateValue = require('./SkipPopulateValue.js');
var get = require('../get.js');
var getDiscriminatorByValue = require('../discriminator/getDiscriminatorByValue.js');
var isPathExcluded = require('../projection/isPathExcluded.js');
var getSchemaTypes = require('./getSchemaTypes.js');
var getVirtual = require('./getVirtual.js');
var lookupLocalFields = require('./lookupLocalFields.js');
var index$1 = require('../../../../mpath/index.js');
var normalizeRefPath = require('./normalizeRefPath.js');
...
var getModelsMapForPopulate = function getModelsMapForPopulate(model, docs, options) {
  let i;
  let doc;
  ...
  for (i = 0; i < len; i++) {
    ...
    if (isUnderneathDocArray && get(options, 'options.sort') != null) {
      return new index('Cannot populate with `sort` on path ' + options.path +
    ...
    if (virtual && (!localField || !foreignField)) {
      return new index('If you are populating a virtual, you must set the ' +
    ...
    if (Array.isArray(localField) && Array.isArray(foreignField) && localField.length === foreignField.length) {
      ...
      for (let i = 1; i < localField.length; ++i) {
        match[foreignField[i]] = convertTo_id(index$1.get(localField[i], doc, lookupLocalFields), schema);
    ...
    if (localFieldGetters.length > 0 && getters) {
      ...
      const localFieldValue = index$1.get(localField, doc, lookupLocalFields);
      if (Array.isArray(localFieldValue)) {
        const localFieldHydratedValue = index$1.get(localField.split('.').slice(0, -1), hydratedDoc, lookupLocalFields);
      ...
      ret = convertTo_id(index$1.get(localField, doc, lookupLocalFields), schema);
    }
  ...
  function _getModelNames(doc, schema) {
    ...
    if (modelNameFromQuery) {
      ...
      if (options._queryProjection != null && isPathExcluded(options._queryProjection, normalizedRefPath)) {
        throw new index('refPath `' + normalizedRefPath +
          '` must not be excluded in projection, got ' +
          util__default['default'].inspect(options._queryProjection));
      }
...
module.exports = getModelsMapForPopulate;
//# sourceMappingURL=getModelsMapForPopulate.js.map
