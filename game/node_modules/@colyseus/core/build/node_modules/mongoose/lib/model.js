...
var ChangeStream = require('./cursor/ChangeStream.js');
var document = require('./document.js');
var notFound = require('./error/notFound.js');
var divergentArray = require('./error/divergentArray.js');
var require$$0 = require('events');
var buffer = require('./types/buffer.js');
var index = require('./error/index.js');
var overwriteModel = require('./error/overwriteModel.js');
var promise_provider = require('./promise_provider.js');
var query = require('./query.js');
var removeOptions = require('./options/removeOptions.js');
var saveOptions = require('./options/saveOptions.js');
var schema = require('./schema.js');
var serverSelection = require('./error/serverSelection.js');
var validation = require('./error/validation.js');
var version = require('./error/version.js');
var parallelSave = require('./error/parallelSave.js');
var applyQueryMiddleware = require('./helpers/query/applyQueryMiddleware.js');
var applyHooks = require('./helpers/model/applyHooks.js');
var applyMethods = require('./helpers/model/applyMethods.js');
var applyStaticHooks = require('./helpers/model/applyStaticHooks.js');
var applyStatics = require('./helpers/model/applyStatics.js');
var applyWriteConcern = require('./helpers/schema/applyWriteConcern.js');
var assignVals = require('./helpers/populate/assignVals.js');
var castBulkWrite = require('./helpers/model/castBulkWrite.js');
var createPopulateQueryFilter = require('./helpers/populate/createPopulateQueryFilter.js');
var getDefaultBulkwriteResult = require('./helpers/getDefaultBulkwriteResult.js');
var discriminator = require('./helpers/model/discriminator.js');
var each = require('./helpers/each.js');
var get = require('./helpers/get.js');
var getDiscriminatorByValue = require('./helpers/discriminator/getDiscriminatorByValue.js');
var getModelsMapForPopulate = require('./helpers/populate/getModelsMapForPopulate.js');
var immediate = require('./helpers/immediate.js');
var options = require('./options.js');
var isDefaultIdIndex = require('./helpers/indexes/isDefaultIdIndex.js');
var isIndexEqual = require('./helpers/indexes/isIndexEqual.js');
var isPathSelectedInclusive = require('./helpers/projection/isPathSelectedInclusive.js');
var leanPopulateMap = require('./helpers/populate/leanPopulateMap.js');
var modifiedPaths = require('./helpers/update/modifiedPaths.js');
var parallelLimit = require('./helpers/parallelLimit.js');
var removeDeselectedForeignField = require('./helpers/populate/removeDeselectedForeignField.js');
var util = require('util');
var utils = require('./utils.js');
var symbols = require('./helpers/symbols.js');
...
var model = _commonjsHelpers.createCommonjsModule(function (module, exports) {
...
const arrayAtomicsSymbol = symbols['default'].arrayAtomicsSymbol;
const modelCollectionSymbol = Symbol('mongoose#Model#collection');
const modelDbSymbol = Symbol('mongoose#Model#db');
const modelSymbol = symbols['default'].modelSymbol;
...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  if (this.isNew) {
    ...
    if ((obj || {})._id === void 0) {
      ...
      immediate(function() {
        callback(new index('document must have an _id before saving'));
      });
      return;
    }
    ...
    if (delta) {
      if (delta instanceof index) {
        callback(delta);
        return;
      }

      const where = this.$__where(delta[0]);
      if (where instanceof index) {
        callback(where);
        return;
      }
      ...
      this.constructor.exists(where, optionsWithCustomValues)
        .then((documentExists) => {
          if (!documentExists) {
            throw new notFound(this.$__where(), this.constructor.modelName);
          }
  ...
  this.$__handleSave(options, (error, result) => {
    ...
    if (get(options, 'safe.w') !== 0 && get(options, 'w') !== 0) {
      ...
      if (this.$__.version && !this.$__.inserting) {
        ...
        const key = this.$__schema.options.versionKey;
        const version$1 = this.$__getValue(key) || 0;
        if (numAffected <= 0) {
          // the update failed. pass an error back
          this.$__undoReset();
          const err = this.$__.$versionError ||
            new version(this, version$1, this.$__.modifiedPaths);
          return callback(err);
        }

        // increment version if was successful
        if (doIncrement) {
          this.$__setValue(key, version$1 + 1);
        }
      }
      if (result != null && numAffected <= 0) {
        this.$__undoReset();
        error = new notFound(result.$where,
...
function generateVersionError(doc, modifiedPaths) {
  ...
  const version$1 = doc.$__getValue(key) || 0;
  return new version(doc, version$1, modifiedPaths);
}
...
Model.prototype.save = function(options, fn) {
  ...
  if (this.$__.saving) {
    parallelSave$1 = new parallelSave(this);
  } else {
    this.$__.saving = new parallelSave(this);
  }
  ...
  return this.constructor.db.base._promiseOrCallback(fn, cb => {
    ...
    if (parallelSave$1) {
      this.$__handleReject(parallelSave$1);
      return cb(parallelSave$1);
    }
...
Model.prototype.$__delta = function() {
  ...
  for (; d < len; ++d) {
    ...
    } else if (value[buffer.pathSymbol] && Buffer.isBuffer(value)) {
      // MongooseBuffer
  ...
  if (divergent.length) {
    return new divergentArray(divergent);
  }
...
Model.prototype.$__where = function _where(where) {
  ...
  if (this._doc._id === void 0) {
    return new index('No _id found on document!');
  }
...
Model.discriminator = function(name, schema$1, value) {
  let model;
  if (typeof name === 'function') {
    model = name;
    name = utils.getFunctionName(model);
    if (!(model.prototype instanceof Model)) {
      throw new index('The provided class ' + name + ' must extend Model');
    }
  }

  _checkContext(this, 'discriminator');

  if (utils.isObject(schema$1) && !schema$1.instanceOfSchema) {
    schema$1 = new schema(schema$1);
  }

  schema$1 = discriminator(this, name, schema$1, value, true);
  if (this.db.models[name]) {
    throw new overwriteModel(name);
  }

  schema$1.$isRootDiscriminator = true;
  schema$1.$globalPluginsApplied = true;

  model = this.db.model(model || name, schema$1, this.$__collection.name);
  ...
  applyMethods(d, schema$1);
  applyStatics(d, schema$1);

  if (this[subclassedSymbol] != null) {
    for (const submodel of this[subclassedSymbol]) {
      submodel.discriminators = submodel.discriminators || {};
      submodel.discriminators[name] =
        model.__subclass(model.db, schema$1, submodel.collection.name);
    }
  }
...
function _checkContext(ctx, fnName) {
  ...
  if (ctx == null || ctx === _commonjsHelpers.commonjsGlobal) {
    throw new index('`Model.' + fnName + '()` cannot run without a ' +
...
Model.init = function init(callback) {
  ...
  const Promise = promise_provider.get();
...
Model.createCollection = function createCollection(options, callback) {
  ...
  if (typeof options === 'string') {
    throw new index('You can\'t specify a new collection name in Model.createCollection.' +
...
const _handleSafe = util__default['default'].deprecate(function _handleSafe(options) {
  if (options.safe) {
    ...
    args.forEach(doc => {
      toExecute.push(callback => {
        const Model = this.discriminators && doc[discriminatorKey] != null ?
          ...
          this;
        if (Model == null) {
          throw new index(`Discriminator "${doc[discriminatorKey]}" not ` +
...
Model.buildBulkWriteOperations = function buildBulkWriteOperations(documents, options) {
  ...
  const writeOperations = documents.reduce((accumulator, document$1, i) => {
    if (!options.skipValidation) {
      if (!(document$1 instanceof document)) {
        throw new Error(`documents.${i} was not a mongoose document, documents must be an array of mongoose documents (instanceof mongoose.Document).`);
      }
      const validationError = document$1.validateSync();
      if (validationError) {
        throw validationError;
      }
    }

    const delta = document$1.$__delta();
    const where = document$1.$__where(delta[0]);
    const changes = delta[1];

    _applyCustomWhere(document$1, where);

    document$1.$__version(where, delta);

    if (document$1.isNew) {
      accumulator.push({
        insertOne: { document: document$1 }
      });
...
Model.hydrate = function(obj, projection) {
  ...
  const model = queryhelpers.createModel(this, obj, projection);
  model.init(obj);
  return model;
};
...
Model.aggregate = function aggregate$1(pipeline, callback) {
  _checkContext(this, 'aggregate');

  if (arguments.length > 2 || get(pipeline, 'constructor.name') === 'Object') {
    throw new index('Mongoose 5.x disallows passing a spread of operators ' +
  ...
  const aggregate$1 = new aggregate(pipeline || []);
  aggregate$1.model(this);
...
Model.geoSearch = function(conditions, options, callback) {
  ...
  return this.db.base._promiseOrCallback(callback, cb => {
    ...
    if (conditions === undefined || !utils.isObject(conditions)) {
      error = new index('Must pass conditions to geoSearch');
    } else if (!options.near) {
      error = new index('Must specify the near option in geoSearch');
    } else if (!Array.isArray(options.near)) {
      error = new index('near option must be an array [x, y]');
    }
...
Model.$handleCallbackError = function(callback) {
  ...
  if (typeof callback !== 'function') {
    throw new index('Callback must be a function, got ' + callback);
  }
...
Model.$wrapCallback = function(callback) {
  const serverSelectionError = new serverSelection();
  const _this = this;
...
if (util__default['default'].inspect.custom) {
  ...
  */
  ...
  Model[util__default['default'].inspect.custom] = Model.inspect;
}
