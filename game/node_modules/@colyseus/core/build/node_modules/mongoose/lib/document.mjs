...
import mixed from './schema/mixed.mjs';
import objectExpected from './error/objectExpected.mjs';
import objectParameter from './error/objectParameter.mjs';
import parallelValidate from './error/parallelValidate.mjs';
import schema from './schema.mjs';
import strict from './error/strict.mjs';
import validation from './error/validation.mjs';
import validator from './error/validator.mjs';
import virtualtype from './virtualtype.mjs';
import promiseOrCallback from './helpers/promiseOrCallback.mjs';
import cleanModifiedSubpaths from './helpers/document/cleanModifiedSubpaths.mjs';
import compile_1 from './helpers/document/compile.mjs';
import common from './helpers/common.mjs';
import get from './helpers/get.mjs';
import getEmbeddedDiscriminatorPath from './helpers/document/getEmbeddedDiscriminatorPath.mjs';
import handleSpreadDoc from './helpers/document/handleSpreadDoc.mjs';
import idGetter_1 from './plugins/idGetter.mjs';
import immediate from './helpers/immediate.mjs';
import isDefiningProjection from './helpers/projection/isDefiningProjection.mjs';
import isExclusive from './helpers/projection/isExclusive.mjs';
import util from 'util';
import options from './options.mjs';
import mpath from '../../mpath/index.mjs';
import queryhelpers from './queryhelpers.mjs';
import utils from './utils.mjs';
import isPromise_1 from './helpers/isPromise.mjs';
import symbols from './helpers/symbols.mjs';
import symbols$1 from './schema/symbols.mjs';
import embedded from './types/embedded.mjs';
import './types/array.mjs';
import './types/documentarray.mjs';
...
const compile = compile_1.compile;
const defineKey = compile_1.defineKey;
const flatten = common.flatten;
...
const documentArrayParent = symbols.documentArrayParent;
const documentIsModified = symbols.documentIsModified;
const documentModifiedPaths = symbols.documentModifiedPaths;
const documentSchemaSymbol = symbols.documentSchemaSymbol;
const getSymbol = symbols.getSymbol;
const populateModelSymbol = symbols.populateModelSymbol;
const scopeSymbol = symbols.scopeSymbol;
const schemaMixedSymbol = symbols$1.schemaMixedSymbol;
let Embedded;
...
function Document(obj, fields, skipId, options) {
  ...
  if (obj != null && typeof obj !== 'object') {
    throw new objectParameter(obj, 'obj', 'Document');
  }

  const schema$1 = this.$__schema;

  if (typeof fields === 'boolean' || fields === 'throw') {
    this.$__.strictMode = fields;
    fields = undefined;
  } else {
    this.$__.strictMode = schema$1.options.strict;
    this.$__.selected = fields;
  }

  const requiredPaths = schema$1.requiredPaths(true);
      ...
      if (obj[i] === null) {
        ...
        if (schema && !wasPopulated) {
          try {
            ...
            self.invalidate(e.path, new validator({
              path: e.path,
              message: e.message,
              type: 'cast',
              value: e.value,
...
Document.prototype.$session = function $session(session) {
  ...
  if (session != null && session.hasEnded) {
    throw new error('Cannot set a document\'s session to a session that has ended. Make sure you haven\'t ' +
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  const strict$1 = 'strict' in options
    ? options.strict
    : this.$__.strictMode;
  ...
  if (path == null) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      const someCondition = typeof path[key] === 'object' &&
                            ...
                            !(this.$__path(pathName) instanceof mixed) &&
      ...
      if (someCondition) {
        ...
        } else if (strict$1 === 'throw') {
          if (pathtype === 'nested') {
            throw new objectExpected(key, path[key]);
          } else {
            throw new strict(key);
          }
        }
  ...
  if (pathType === 'nested' && val) {
    ...
    this.invalidate(path, new error.CastError('Object', val, path));
    return this;
  }
  ...
  if (pathType === 'adhocOrUndefined' && strict$1) {
  ...
  try {
    ...
    if (e instanceof error.StrictModeError && e.isImmutableError) {
      this.invalidate(path, e);
    } else if (e instanceof error.CastError) {
      this.invalidate(e.path, e);
      if (e.$originalErrorPath) {
        this.invalidate(path,
          new error.CastError(schema.instance, val, path, e.$originalErrorPath));
      }
    } else {
      this.invalidate(path,
        new error.CastError(schema.instance, val, path, e));
    }
    shouldSet = false;
  }
...
Document.prototype.validate = function(pathsToValidate, options, callback) {
  let parallelValidate$1;
  this.$op = 'validate';

  if (this.ownerDocument != null) ; else if (this.$__.validating) {
    parallelValidate$1 = new parallelValidate(this, {
      parentStack: options && options.parentStack,
      conflictStack: this.$__.validating.stack
    });
  } else {
    this.$__.validating = new parallelValidate(this, { parentStack: options && options.parentStack });
  }
  ...
  return promiseOrCallback(callback, cb => {
    if (parallelValidate$1 != null) {
      return cb(parallelValidate$1);
    }
    ...
    if (validationError) {
      for (const key in validationError.errors) {
        ...
        if (!this[documentArrayParent] &&
            validationError.errors[key] instanceof error.CastError) {
...
Document.prototype.validateSync = function(pathsToValidate, options) {
  ...
  if (err) {
    for (const key in err.errors) {
      // Make sure cast errors persist
      if (err.errors[key] instanceof error.CastError) {
...
Document.prototype.invalidate = function(path, err, val, kind) {
  if (!this.$__.validationError) {
    this.$__.validationError = new validation(this);
  }
...
Document.prototype.$__setSchema = function(schema) {
  schema.plugin(idGetter_1, { deduplicate: true });
...
function throwErrorIfPromise(path, transformedValue) {
  if (isPromise_1(transformedValue)) {
...
Document.ValidationError = validation;
