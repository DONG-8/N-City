...
var index$1 = require('../../mongodb/index.js');
var _package = require('../package.json.js');
var utils = require('./utils.js');
var index$2 = require('../../mongodb/lib/core/index.js');
...
const parseConnectionString = index$2.parseConnectionString;

const arrayAtomicsSymbol = symbols['default'].arrayAtomicsSymbol;
const sessionNewDocuments = symbols['default'].sessionNewDocuments;
...
function Connection(base) {
  ...
  this._readyState = connectionstate.disconnected;
...
Object.defineProperty(Connection.prototype, 'readyState', {
  ...
  set: function(val) {
    ...
    if (this._readyState !== val) {
      ...
      if (connectionstate.connected === val) {
        this._hasOpened = true;
      }

      this.emit(connectionstate[val]);
    }
  }
});
...
function _wrapConnHelper(fn) {
  return function() {
    ...
    const disconnectedError = new index('Connection ' + this.id +
    ...
    return promiseOrCallback(cb, cb => {
      ...
      // Re: gh-8534
      immediate(() => {
        if (this.readyState === connectionstate.connecting && this._shouldBufferCommands()) {
          this._queue.push({ fn: fn, ctx: this, args: argsWithoutCb.concat([cb]) });
        } else if (this.readyState === connectionstate.disconnected && this.db == null) {
...
Connection.prototype.onOpen = function() {
  this.readyState = connectionstate.connected;
...
Connection.prototype.openUri = function(uri, options, callback) {
  ...
  if (['string', 'number'].indexOf(typeof options) !== -1) {
    throw new index('Mongoose 5.x no longer supports ' +
  ...
  if (typeof uri !== 'string') {
    throw new index('The `uri` parameter to `openUri()` must be a ' +
      `string, got "${typeof uri}". Make sure the first parameter to ` +
      '`mongoose.connect()` or `mongoose.createConnection()` is a string.');
  }

  if (callback != null && typeof callback !== 'function') {
    throw new index('3rd parameter to `mongoose.connect()` or ' +
      '`mongoose.createConnection()` must be a function, got "' +
      typeof callback + '"');
  }

  if (this.readyState === connectionstate.connecting || this.readyState === connectionstate.connected) {
    if (this._connectionString !== uri) {
      throw new index('Can\'t call `openUri()` on an active connection with ' +
  ...
  if (!('promiseLibrary' in options)) {
    options.promiseLibrary = promise_provider.get();
  }
  ...
  const promise = new Promise((resolve, reject) => {
    const client = new index$1.MongoClient(uri, options);
    _this.client = client;
  ...
  this.$initialConnection = Promise.all([promise, parsePromise]).
    then(res => res[0]).
      ...
      this.readyState = connectionstate.disconnected;
...
function _setClient(conn, client, options, dbName) {
  ...
  const _handleReconnect = () => {
    ...
    if (conn.readyState !== connectionstate.connected) {
      conn.readyState = connectionstate.connected;
      conn.emit('reconnect');
    ...
    if (type === 'Single') {
      ...
      client.on('serverDescriptionChanged', ev => {
        ...
        if (newDescription.type === 'Standalone') {
          ...
          conn.readyState = connectionstate.disconnected;
        }
      });
      ...
      client.on('topologyDescriptionChanged', ev => {
        ...
        if (conn.readyState === connectionstate.connected && allServersDisconnected) {
          // Implicitly emits 'disconnected'
          conn.readyState = connectionstate.disconnected;
        } else if (conn.readyState === connectionstate.disconnected && !allServersDisconnected) {
          _handleReconnect();
        }
      });
...
Connection.prototype._close = function(force, callback) {
  ...
  switch (this.readyState) {
    ...
    case connectionstate.connected:
      this.readyState = connectionstate.disconnecting;
      this.doClose(force, function(err) {
        if (err) {
          return callback(err);
        }
...
Connection.prototype.model = function(name, schema$1, collection, options) {
  if (!(this instanceof Connection)) {
    throw new index('`connection.model()` should not be run with ' +
  ...
  if (schema$1 && !schema$1.instanceOfSchema) {
  ...
  if (this.models[name] && !collection && opts.overwriteModels !== true) {
    ...
    if (schema$1 && schema$1.instanceOfSchema && schema$1 !== this.models[name].schema) {
      throw new index.OverwriteModelError(name);
    }
  ...
  if (schema$1 && schema$1.instanceOfSchema) {
    applyPlugins(schema$1, this.plugins, null, '$connectionPluginsApplied');

    // compile a model
    model = this.base.model(fn || name, schema$1, collection, opts);
  ...
  if (this.models[name] && collection) {
    ...
    schema$1 = model.prototype.schema;
    const sub = model.__subclass(this, schema$1, collection);
  ...
  if (!model) {
    throw new index.MissingSchemaError(name);
  }
  ...
  this.models[name] = model.__subclass(this, schema$1, collection);
  return this.models[name];
};
...
Connection.prototype.watch = function(pipeline, options) {
  ...
  const changeStreamThunk = cb => {
    immediate(() => {
      if (this.readyState === connectionstate.connecting) {
        this.once('open', function() {
...
Connection.prototype.setClient = function setClient(client) {
  if (!(client instanceof index$1.MongoClient)) {
    throw new index('Must call `setClient()` with an instance of MongoClient');
  }
  if (this.client != null || this.readyState !== connectionstate.disconnected) {
    throw new index('Cannot call `setClient()` on a connection that is already connected.');
  }
  if (!client.isConnected()) {
    throw new index('Cannot call `setClient()` with a MongoClient that is not connected.');
  }
