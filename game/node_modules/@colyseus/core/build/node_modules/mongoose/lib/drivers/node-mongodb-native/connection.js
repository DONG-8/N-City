...
var timers = require('../../helpers/timers.js');
...
const setTimeout = timers['default'].setTimeout;
...
function NativeConnection() {
  connection$1.apply(this, arguments);
...
NativeConnection.STATES = connectionstate;

/*!
 * Inherits from Connection.
 */

NativeConnection.prototype.__proto__ = connection$1.prototype;
...
NativeConnection.prototype.useDb = function(name, options) {
  ...
  if (this.db && this._readyState === connectionstate.connected) {
    wireup();
  } else {
...
function listen(conn) {
  ...
  if (!conn.client.s.options.useUnifiedTopology) {
    ...
    conn.db.on('open', function(err, db) {
      if (connectionstate.disconnected === conn.readyState && db && db.databaseName) {
        conn.readyState = connectionstate.connected;
        conn.emit('reconnect');
        conn.emit('reconnected');
      }
