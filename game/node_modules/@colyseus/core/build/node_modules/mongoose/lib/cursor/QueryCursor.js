...
var eachAsync = require('../helpers/cursor/eachAsync.js');
...
util__default['default'].inherits(QueryCursor, Readable);
...
function _next(ctx, cb) {
  ...
  if (ctx.cursor) {
    if (ctx.query._mongooseOptions.populate && !ctx._pop) {
      ctx._pop = queryhelpers.preparePopulationOptionsMQ(ctx.query,
...
function _create(ctx, doc, populatedIds, cb) {
  const instance = queryhelpers.createModel(ctx.query.model, doc, ctx.query._fields);
...
var QueryCursor_1 = QueryCursor;

module.exports = QueryCursor_1;
//# sourceMappingURL=QueryCursor.js.map
