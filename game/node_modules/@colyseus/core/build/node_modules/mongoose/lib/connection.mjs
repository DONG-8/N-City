...
import core from '../../mongodb/lib/core/index.mjs';
...
const sessionNewDocuments = symbols.sessionNewDocuments;
...
function _wrapConnHelper(fn) {
  return function() {
    ...
    const disconnectedError = new error('Connection ' + this.id +
...
Connection.prototype.openUri = function(uri, options, callback) {
  ...
  if (['string', 'number'].indexOf(typeof options) !== -1) {
    throw new error('Mongoose 5.x no longer supports ' +
  ...
  if (typeof uri !== 'string') {
    throw new error('The `uri` parameter to `openUri()` must be a ' +
      `string, got "${typeof uri}". Make sure the first parameter to ` +
      '`mongoose.connect()` or `mongoose.createConnection()` is a string.');
  }

  if (callback != null && typeof callback !== 'function') {
    throw new error('3rd parameter to `mongoose.connect()` or ' +
  ...
  if (this.readyState === connectionstate.connecting || this.readyState === connectionstate.connected) {
    if (this._connectionString !== uri) {
      throw new error('Can\'t call `openUri()` on an active connection with ' +
...
Connection.prototype.model = function(name, schema$1, collection, options) {
  if (!(this instanceof Connection)) {
    throw new error('`connection.model()` should not be run with ' +
...
Connection.prototype.watch = function(pipeline, options) {
  ...
  const changeStream = new ChangeStream_1(changeStreamThunk, pipeline, options);
  return changeStream;
};
...
Connection.prototype.setClient = function setClient(client) {
  if (!(client instanceof mongodb.MongoClient)) {
    throw new error('Must call `setClient()` with an instance of MongoClient');
  }
  if (this.client != null || this.readyState !== connectionstate.disconnected) {
    throw new error('Cannot call `setClient()` on a connection that is already connected.');
  }
  if (!client.isConnected()) {
    throw new error('Cannot call `setClient()` with a MongoClient that is not connected.');
  }
