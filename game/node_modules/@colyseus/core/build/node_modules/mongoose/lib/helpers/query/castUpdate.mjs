...
import number from '../../cast/number.mjs';
import cast from '../../cast.mjs';
import getEmbeddedDiscriminatorPath from './getEmbeddedDiscriminatorPath.mjs';
import handleImmutable from './handleImmutable.mjs';
import moveImmutableProperties from '../update/moveImmutableProperties.mjs';
import symbols from '../../schema/symbols.mjs';
import setDottedPath from '../path/setDottedPath.mjs';
import utils from '../../utils.mjs';

const schemaMixedSymbol = symbols.schemaMixedSymbol;
...
var castUpdate = function castUpdate(schema, obj, options, context, filter) {
  if (obj == null) {
    return undefined;
  }
  options = options || {};
...
function walkUpdatePath(schema, obj, op, options, context, filter, pref) {
  ...
  while (i--) {
    ...
    if (val && val.constructor.name === 'Object') {
      ...
      if (op !== '$setOnInsert' &&
          handleImmutable(schematype, strict$1, obj, key, prefix + key, context)) {
        continue;
      }
...
function _handleCastError(error, query, key, aggregatedError) {
  ...
  aggregatedError = aggregatedError || new validation();
...
function castUpdateVal(schema, val, op, $conditional, context, path) {
  if (!schema) {
    ...
    if (op in numberOps) {
      try {
        ...
        throw new cast$1('number', val, path);
      }
    }
    return val;
  }
  ...
  if (op in numberOps) {
    ...
    if (val == null) {
      throw new cast$1('number', val, schema.path);
    }
    if (op === '$inc') {
...
//# sourceMappingURL=castUpdate.mjs.map
