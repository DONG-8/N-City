...
import read_preference from './read_preference.mjs';
import cursor from '../cursor.mjs';
import utils$1 from '../connection/utils.mjs';
import logger from '../connection/logger.mjs';
import error from '../error.mjs';
import server from './server.mjs';
import replset_state from './replset_state.mjs';
import shared from './shared.mjs';
import utils$2 from '../utils.mjs';
...
const CoreCursor = cursor.CoreCursor;
const retrieveBSON = utils$1.retrieveBSON;

const MongoError = error.MongoError;


const Timeout = shared.Timeout;
const Interval = shared.Interval;
const SessionMixins = shared.SessionMixins;
const isRetryableWritesSupported = shared.isRetryableWritesSupported;
const relayEvents = utils$2.relayEvents;
const BSON = retrieveBSON();
const getMMAPError = shared.getMMAPError;
const makeClientMetadata = utils$2.makeClientMetadata;
const legacyIsRetryableWriteError = shared.legacyIsRetryableWriteError;
const now = utils.now;
const calculateDurationInMs = utils.calculateDurationInMs;
...
var ReplSet = function(seedlist, options) {
  ...
  // Create a logger
  var logger$1 = logger('ReplSet', options);
  ...
  this.s = {
    ...
    seedlist: seedlist,
    // Replicaset state
    replicaSetState: new replset_state({
      id: this.id,
...
function connectNewServers(self, servers, callback) {
  ...
  function execute(_server, i) {
    setTimeout(function() {
      ...
      // Add temp handlers
      server$1.once('connect', _handleEvent(self, 'connect'));
      server$1.once('close', _handleEvent(self, 'close'));
      server$1.once('timeout', _handleEvent(self, 'timeout'));
      server$1.once('error', _handleEvent(self, 'error'));
      server$1.once('parseError', _handleEvent(self, 'parseError'));

      // SDAM Monitoring events
      server$1.on('serverOpening', e => self.emit('serverOpening', e));
      server$1.on('serverDescriptionChanged', e => self.emit('serverDescriptionChanged', e));
      server$1.on('serverClosed', e => self.emit('serverClosed', e));

      // Command Monitoring events
      relayEvents(server$1, self, ['commandStarted', 'commandSucceeded', 'commandFailed']);

      self.s.connectingServers.push(server$1);
      server$1.connect(self.s.connectOptions);
    }, i);
  }
...
function shouldTriggerConnect(self) {
  ...
  const readPreferenceSecondary =
    ...
    self.s.connectOptions.readPreference.equals(read_preference.secondary);
...
ReplSet.prototype.isConnected = function(options) {
  ...
  if (options.readPreference && options.readPreference.equals(read_preference.secondary)) {
  ...
  if (options.readPreference && options.readPreference.equals(read_preference.primaryPreferred)) {
    return this.s.replicaSetState.hasSecondary() || this.s.replicaSetState.hasPrimary();
  }

  if (options.readPreference && options.readPreference.equals(read_preference.secondaryPreferred)) {
    return this.s.replicaSetState.hasSecondary() || this.s.replicaSetState.hasPrimary();
  }
...
ReplSet.prototype.selectServer = function(selector, options, callback) {
  ...
  let readPreference;
  if (selector instanceof read_preference) {
    readPreference = selector;
  } else {
    readPreference = options.readPreference || read_preference.primary;
  }
  ...
  const _selectServer = () => {
    ...
    const server$1 = this.s.replicaSetState.pickServer(readPreference);
    if (server$1 == null) {
      setTimeout(_selectServer, SERVER_SELECTION_INTERVAL_MS);
      return;
    }

    if (!(server$1 instanceof server)) {
      lastError = server$1;
      setTimeout(_selectServer, SERVER_SELECTION_INTERVAL_MS);
      return;
    }

    if (this.s.debug) this.emit('pickedServer', options.readPreference, server$1);
...
ReplSet.prototype.command = function(ns, cmd, options, callback) {
  ...
  var readPreference = options.readPreference ? options.readPreference : read_preference.primary;
  ...
  // Pick a server
  var server$1 = this.s.replicaSetState.pickServer(readPreference);
  // We received an error, return it
  if (!(server$1 instanceof server)) return callback(server$1);
  // Emit debug event
  if (self.s.debug) self.emit('pickedServer', read_preference.primary, server$1);
  ...
  // Execute the command
  server$1.command(ns, cmd, options, cb);
};
...
export default replset;
//# sourceMappingURL=replset.mjs.map
