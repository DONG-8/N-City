...
const MongoParseError = error.MongoParseError;
...
function parseSrvConnectionString(uri, options, callback) {
  const result = URL__default['default'].parse(uri, true);
  ...
  dns__default['default'].resolveSrv(`_mongodb._tcp.${lookupAddress}`, (err, addresses) => {
    ...
    dns__default['default'].resolveTxt(lookupAddress, (err, record) => {
      if (err) {
      ...
      if (record) {
        ...
        record = qs__default['default'].parse(record[0].join(''));
      ...
      result.search = qs__default['default'].stringify(result.query);

      const finalString = URL__default['default'].format(result);
...
function parseQueryString(query, options) {
  const result = {};
  let parsedQueryString = qs__default['default'].parse(query);
...
function parseConnectionString(uri, options, callback) {
  ...
  try {
    URL__default['default'].parse(uri);
  } catch (e) {
  ...
  const auth = { username: null, password: null, db: db && db !== '' ? qs__default['default'].unescape(db) : null };
  ...
  if (authorityParts.length > 1) {
    ...
    if (!auth.username) auth.username = qs__default['default'].unescape(authParts[0]);
    if (!auth.password) auth.password = authParts[1] ? qs__default['default'].unescape(authParts[1]) : null;
  }
  ...
  const hosts = authorityParts
    ...
    .split(',')
    .map(host => {
      let parsedHost = URL__default['default'].parse(`mongodb://${host}`);
      if (parsedHost.path === '/:') {
      ...
      if (host.match(/\.sock/)) {
        parsedHost.hostname = qs__default['default'].unescape(host);
...
module.exports = uri_parser;
//# sourceMappingURL=uri_parser.js.map
