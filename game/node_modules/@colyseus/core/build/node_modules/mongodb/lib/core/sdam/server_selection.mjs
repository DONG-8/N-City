...
function readPreferenceServerSelector(readPreference) {
  ...
  return function(topologyDescription, servers) {
    ...
    if (mode === read_preference.PRIMARY) {
      return servers.filter(primaryFilter);
    }

    if (mode === read_preference.PRIMARY_PREFERRED) {
    ...
    const filter = mode === read_preference.NEAREST ? nearestFilter : secondaryFilter;
    ...
    if (mode === read_preference.SECONDARY_PREFERRED && selectedServers.length === 0) {
      return servers.filter(primaryFilter);
    }
...
export default server_selection;
