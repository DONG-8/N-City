...
import message_stream from './message_stream.mjs';
import error from '../core/error.mjs';
import command_result from '../core/connection/command_result.mjs';
import stream_description from './stream_description.mjs';
import wireprotocol from '../core/wireprotocol/index.mjs';
import apm from '../core/connection/apm.mjs';
import sessions from '../core/sessions.mjs';
import utils$1 from '../core/utils.mjs';
import utils from '../utils.mjs';

const MongoError = error.MongoError;
const MongoNetworkError = error.MongoNetworkError;
const MongoNetworkTimeoutError = error.MongoNetworkTimeoutError;
const MongoWriteConcernError = error.MongoWriteConcernError;

const StreamDescription = stream_description.StreamDescription;


const updateSessionFromResponse = sessions.updateSessionFromResponse;
const uuidV4 = utils$1.uuidV4;
const now = utils.now;
...
class Connection extends require$$0 {
  constructor(stream, options) {
    ...
    this[kQueue] = new Map();
    this[kMessageStream] = new message_stream(options);
  ...
  command(ns, cmd, options, callback) {
    wireprotocol.command(makeServerTrampoline(this), ns, cmd, options, callback);
  }

  query(ns, cmd, cursorState, options, callback) {
    wireprotocol.query(makeServerTrampoline(this), ns, cmd, cursorState, options, callback);
  }

  getMore(ns, cursorState, batchSize, options, callback) {
    wireprotocol.getMore(makeServerTrampoline(this), ns, cursorState, batchSize, options, callback);
  }

  killCursors(ns, cursorState, callback) {
    wireprotocol.killCursors(makeServerTrampoline(this), ns, cursorState, callback);
  }

  insert(ns, ops, options, callback) {
    wireprotocol.insert(makeServerTrampoline(this), ns, ops, options, callback);
  }

  update(ns, ops, options, callback) {
    wireprotocol.update(makeServerTrampoline(this), ns, ops, options, callback);
  }

  remove(ns, ops, options, callback) {
    wireprotocol.remove(makeServerTrampoline(this), ns, ops, options, callback);
  }
}
