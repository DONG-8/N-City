...
var native_topology = require('../topologies/native_topology.js');
...
var events = require('../cmap/events.js');
var mongo_client = require('../mongo_client.js');
var url_parser = require('../url_parser.js');
...
const MongoCredentials = index.MongoCredentials;
const MongoError = index.MongoError;


const parse = index.parseConnectionString;

const ReadPreference = index.ReadPreference;


const ServerSessionPool = index.Sessions.ServerSessionPool;
const emitDeprecationWarning = utils.emitDeprecationWarning;
const emitWarningOnce = utils.emitWarningOnce;


const CMAP_EVENT_NAMES = events.CMAP_EVENT_NAMES;
...
function assignTopology(client, topology) {
  ...
  if (!(topology instanceof native_topology)) {
...
function resolveTLSOptions(options) {
  ...
  ['sslCA', 'sslKey', 'sslCert'].forEach(optionName => {
    if (options[optionName]) {
      options[optionName] = fs__default['default'].readFileSync(options[optionName]);
    }
  });
}
...
function connect(mongoClient, url, options, callback) {
  ...
  parseFn(url, options, (err, _object) => {
    ...
    mongoClient.s.writeConcern = write_concern.fromOptions(_finalOptions);
...
function createTopology(mongoClient, topologyType, options, callback) {
  ...
  if (options.autoEncryption != null) {
    const Encrypter = encrypter.Encrypter;
  ...
  if (topologyType === 'mongos') {
    ...
    topology = new native_topology(options.servers, options);
...
function createUnifiedOptions(finalOptions, options) {
  ...
  const optionsWriteConcern = write_concern.fromOptions(options);
...
function transformUrlOptions(_object) {
  ...
  if (object.readConcernLevel) {
    object.readConcern = new read_concern(object.readConcernLevel);
  }
...
var connect_1 = { validOptions, connect };

module.exports = connect_1;
