...
var operation = require('../operations/operation.js');
var async_iterator = require('../async/async_iterator.js');
...
const collationNotSupported = utils$2.collationNotSupported;

const isUnifiedTopology = utils$2.isUnifiedTopology;

const Readable = Stream__default['default'].Readable;
const SUPPORTS = utils.SUPPORTS;
...
class CoreCursor extends Readable {
  ...
  _initializeCursor(callback) {
    ...
    if (isUnifiedTopology(cursor.topology) && cursor.topology.shouldCheckForSessionSupport()) {
      cursor.topology.selectServer(read_preference.primaryPreferred, err => {
        if (err) {
          callback(err);
          return;
        }
    ...
    if (cursor.operation) {
      ...
      execute_operation(cursor.topology, cursor.operation, (err, result) => {
        if (err) {
          done(err);
          return;
        }
...
if (SUPPORTS.ASYNC_ITERATOR) {
  CoreCursor.prototype[Symbol.asyncIterator] = async_iterator['default'].asyncIterator;
}
