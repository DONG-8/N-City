...
var url_parser = function(url, options, callback) {
  if (typeof options === 'function') (callback = options), (options = {});
  options = options || {};

  let result;
  try {
    result = URL__default['default'].parse(url, true);
  } catch (e) {
  ...
  dns__default['default'].resolveSrv(srvAddress, function(err, addresses) {
    ...
    dns__default['default'].resolveTxt(result.host, function(err, record) {
      ...
      if (record) {
        ...
        const parsedRecord = qs__default['default'].parse(record);
...
function parseConnectionString(url, options) {
  ...
  // Url parser result
  let result = URL__default['default'].parse(url, true);
  ...
  for (let i = 0; i < hosts.length; i++) {
    let r = URL__default['default'].parse(f('mongodb://%s', hosts[i].trim()));
  ...
  urlOptions.forEach(function(opt) {
    ...
    switch (name) {
      ...
      case 'readConcernLevel':
        dbOptions.readConcern = new read_concern(value);
        break;
