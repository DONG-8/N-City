...
var index$1 = require('../../../safe-buffer/index.js');
var fs = require('fs');
...
var writeFile = function(self, file, options, callback) {
  if (typeof file === 'string') {
    fs__default['default'].open(file, 'r', function(err, fd) {
      if (err) return callback(err);
  ...
  self.open(function(err, self) {
    ...
    fs__default['default'].fstat(file, function(err, stats) {
      if (err) return callback(err, self);
      ...
      var writeChunk = function() {
        ...
        // Read the file
        fs__default['default'].read(file, _buffer, 0, _buffer.length, offset, function(err, bytesRead, data) {
          if (err) return callback(err, self);

          offset = offset + bytesRead;

          // Create a new chunk for the data
          var chunk$1 = new chunk(self, { n: index++ }, self.writeConcern);
          chunk$1.write(data.slice(0, bytesRead), function(err, chunk) {
            if (err) return callback(err, self);
            ...
            chunk.save({}, function(err) {
              ...
              if (offset >= stats.size) {
                fs__default['default'].close(file, function(err) {
                  if (err) return callback(err);
  ...
  if (query != null) {
    collection.findOne(query, options, function(err, doc) {
      ...
      if (self.mode === 'r') {
        ...
        nthChunk(self, lastChunkNumber(self), options, function(err, chunk$1) {
          if (err) return error(err);
          // Set the current chunk
          self.currentChunk = chunk$1 == null ? new chunk(self, { n: 0 }, self.writeConcern) : chunk$1;
...
var nthChunk = function(self, chunkNumber, options, callback) {
  ...
  self
    .chunkCollection()
    .findOne({ files_id: self.fileId, n: chunkNumber }, options, function(err, chunk$1) {
      if (err) return callback(err);

      var finalChunk = chunk$1 == null ? {} : chunk$1;
 ...
 * @ignore
 */
var grid_store = GridStore;

module.exports = grid_store;
//# sourceMappingURL=grid_store.js.map
