...
var defaultAuthProviders$1 = require('../auth/defaultAuthProviders.js');
var auth_provider = require('../auth/auth_provider.js');
var constants = require('../wireprotocol/constants.js');
...
const defaultAuthProviders = defaultAuthProviders$1.defaultAuthProviders;
const AuthContext = auth_provider.AuthContext;

const makeClientMetadata = utils.makeClientMetadata;
const MAX_SUPPORTED_WIRE_VERSION = constants.MAX_SUPPORTED_WIRE_VERSION;
const MAX_SUPPORTED_SERVER_VERSION = constants.MAX_SUPPORTED_SERVER_VERSION;
const MIN_SUPPORTED_WIRE_VERSION = constants.MIN_SUPPORTED_WIRE_VERSION;
const MIN_SUPPORTED_SERVER_VERSION = constants.MIN_SUPPORTED_SERVER_VERSION;
let AUTH_PROVIDERS;
...
function parseSslOptions(family, options) {
  ...
  if (result.servername == null && !net__default['default'].isIP(result.host)) {
...
function makeConnection(family, options, cancellationToken, _callback) {
  ...
  try {
    if (useSsl) {
      socket = tls__default['default'].connect(parseSslOptions(family, options));
      if (typeof socket.disableRenegotiation === 'function') {
        socket.disableRenegotiation();
      }
    } else {
      socket = net__default['default'].createConnection(parseConnectOptions(family, options));
    }
  } catch (err) {
    return callback(err);
  }
